...
function open_database()
{
    try {
        ...
        $conn = new PDO("mysql:server=$serverName ; Database=$database", "$username", "$pw");
...
function findquery(string $query = null)
{
    $database = open_database();
    $found = null;
 ...
 * Start Sessao
 */
function criasessao($login, $nome, $conectado, $url)
{
    ...
    if ($conectado) {
        if (session_status() != PHP_SESSION_ACTIVE) {
            ...
            if ($login == "tmsa.web" || $login == "service.axadmin") {
                $_SESSION['nome'] = "TMSA Web Administrador";
                $_SESSION['login'] = $login;
            }

            // $matusers = find_query("select * from usuarios where cpf = '51647400015' and matricula = '4306'");
            ...
            foreach ($matusers as $matuser) :
                ...
                $_SESSION['login'] = $matuser['login'];
                $_SESSION['iduser'] = $matuser['id'];
                $_SESSION['idempresa'] = $matuser['idempresa'];
                break;
            endforeach;
